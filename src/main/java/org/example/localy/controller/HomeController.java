...
import org.example.localy.service.HomeService;
...
public class HomeController {
    ...
    @GetMapping
    public BaseResponse<HomeResponseDto> getHomeData(@AuthenticationPrincipal Users user) {
    ...
    @Operation(
            summary = "홈 - 최근 북마크 가져오기",
            description = """
          """
    )
    @GetMapping("/recentBookmarks")
    public BaseResponse<List<PlaceDto.BookmarkItem>> getRecentBookmarks(@AuthenticationPrincipal Users user) {

        List<PlaceDto.BookmarkItem> res = homeService.getRecentBookmarks(user);
